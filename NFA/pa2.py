...
class NFA:
	...
	def toDFA(self, dfa_filename):
		...
		self.writeFileDFA(dfa_filename, dfa_start_state, num_dfa_states, dfa_transition_table, dfa_accept_states)
	...
	def writeFileDFA(self, filename, start_state, num_states, transition_table, accept_states):
		...
		for transition in transition_table:
			dfa_file.write(transition[:-3] + " " + transition[-3:] + " " + transition_table[transition] + "\n")

		dfa_file.write(start_state + "\n")
